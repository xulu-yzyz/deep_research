...
def initialize_agents(together_key, composio_key):
    ...
    llm = DeepSeek(
    id="deepseek-chat",   # 或 deepseek-reasoner
...
def research_question(llm, composio_tools, topic, domain, question):
    research_task = Agent(
        ...
        instructions=f"You are a sophisticated research assistant. \
    ...
    research_result: RunOutput = research_task.run(f"Compile final report for topic={topic}, domain={domain}")
...
def compile_report(llm, composio_tools, topic, domain, question_answers):
    with st.spinner("📝 Compiling final report"):
        ...
        compile_result: RunOutput = compile_report_task.run(f"Compile final report for topic={topic}, domain={domain}")
...
if DEEPSEEK_API_KEY and composio_api_key:
    # Initialize agents
    llm, composio_tools = initialize_agents(DEEPSEEK_API_KEY, composio_api_key)
